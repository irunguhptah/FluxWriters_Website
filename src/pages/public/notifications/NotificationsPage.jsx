...
export default function NotificationsPage() {
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message:
        'Your "Machine Learning Research Paper" has been completed by Dr. Alan Turing.',
      time: "2 hours ago",
      read: false,
    ...
    },
    {
      ...
      message:
        'Prof. Adam Smith has requested changes to your "Business Ethics Case Study".',
      time: "1 day ago",
      read: false,
    ...
    },
    {
      ...
      message:
        'Dr. Carl Jung sent a question about your "Psychology Literature Review".',
      time: "3 days ago",
      read: true,
  ...
  const markAsRead = (id) => {
    ...
    // Remove individual mark as read toast - visual change is clear
  };

  // Mark all as read
  const markAllAsRead = () => {
    setNotifications((prev) => prev.map((n) => ({ ...n, read: true })));
    // Keep this toast as it's a bulk action that might not be immediately obvious
  ...
  const deleteNotification = (id) => {
    ...
    // Keep delete toast as it's a destructive action requiring confirmation feedback
  ...
  const getNotificationIcon = (type) => {
    switch (type) {
      ...
      case "revision":
        return (
          <AlertTriangle className="h-5 w-5 text-yellow-500" title="Revision" />
        );
      case "message":
        return (
          <MessageSquare className="h-5 w-5 text-blue-500" title="Message" />
        );
      case "urgent":
        return <Zap className="h-5 w-5 text-red-500" title="Urgent" />;
      case "rating":
        return (
          <Star className="h-5 w-5 text-purple-500" title="Rating request" />
        );
      default:
        return <Bell className="h-5 w-5 text-gray-500" title="Notification" />;
    }
  };
  ...
  const handleFilterChange = (e) => {
    ...
    // Remove filter change toast - selected filter is visually clear
  };
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
      ...
      <div className="border-b border-gray-200 dark:border-gray-800 px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <span
            className="px-2 py-1 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300 text-xs font-medium"

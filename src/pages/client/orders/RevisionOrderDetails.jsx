import React, { useState } from "react";
import {
  RefreshCw,
  FileText,
  AlertTriangle,
  CheckCircle,
  Check,
  X,
  MessageSquare,
  ChevronLeft,
  File,
  User,
  Calendar,
  Paperclip,
  Download,
  Edit,
  Clock,
  ThumbsUp,
  ThumbsDown,
  AlertCircle,
  DollarSign, // <-- Add this
  Info, // <-- Add this
} from "lucide-react";
import toast, { Toaster } from "react-hot-toast";
import { useParams, useNavigate } from "react-router-dom";

export default function RevisionOrderDetails() {
  const { orderId } = useParams();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState("details");
  const [revisionNotes, setRevisionNotes] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedAction, setSelectedAction] = useState(null);

  // Mock data for revision orders
  const orders = [
    {
      id: "ORD-78950",
      title: "Sociology Research Paper",
      writer: "Dr. Emile Durkheim",
      submittedDate: "2023-07-05",
      originalDueDate: "2023-07-15",
      revisionDueDate: "2023-07-18",
      revisionRequest:
        "Add more case studies and expand the methodology section with recent examples from peer-reviewed journals.",
      status: "Pending",
      lastUpdated: "3 days ago",
      price: "$145",
      attachments: [
        { name: "Revised_Paper.docx", size: "2.8 MB", date: "2023-07-05" },
        { name: "Revision_Notes.pdf", size: "0.5 MB", date: "2023-07-05" },
      ],
      messages: [
        {
          id: 1,
          sender: "Dr. Emile Durkheim",
          content:
            "I've addressed your initial feedback and added three new case studies. Please review the methodology section as I've expanded it significantly.",
          timestamp: "2023-07-05 14:30",
          isCustomer: false,
        },
        {
          id: 2,
          sender: "You",
          content:
            "Thanks for the quick turnaround. I'll review and get back to you within 24 hours.",
          timestamp: "2023-07-05 16:45",
          isCustomer: true,
        },
      ],
    },
  ];

  const order = orders.find((o) => o.id === orderId);

  if (!order) {
    return (
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-8 text-center">
        <FileText className="mx-auto h-12 w-12 text-gray-400 dark:text-gray-600" />
        <h3 className="mt-2 text-lg font-medium text-gray-900 dark:text-white">
          Order not found
        </h3>
        <button
          onClick={() => navigate(-1)}
          className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700"
        >
          Go Back
        </button>
      </div>
    );
  }

  const handleApprove = () => {
    setIsSubmitting(true);
    setTimeout(() => {
      toast.success("Revision approved successfully!");
      setIsSubmitting(false);
      setSelectedAction("approved");
      // In a real app, you would update the order status here
    }, 1500);
  };

  const handleReject = () => {
    if (!revisionNotes.trim()) {
      toast.error("Please provide revision notes");
      return;
    }
    setIsSubmitting(true);
    setTimeout(() => {
      toast.success("Revision rejected with your feedback");
      setIsSubmitting(false);
      setSelectedAction("rejected");
      // In a real app, you would update the order status here
    }, 1500);
  };

  const handleDispute = () => {
    if (!revisionNotes.trim()) {
      toast.error("Please explain the dispute reason");
      return;
    }
    setIsSubmitting(true);
    setTimeout(() => {
      toast("Dispute submitted for review", { icon: "⚠️" });
      setIsSubmitting(false);
      setSelectedAction("disputed");
      // In a real app, you would update the order status here
    }, 1500);
  };

  const handleDownload = (file) => {
    toast.success(`Downloading ${file.name}...`);
  };

  const handleChat = () => {
    toast("Opening chat with writer...", { icon: "💬" });
  };

  return (
    <div className="max-w-5xl w-full mx-auto p-6 bg-white dark:bg-gray-900 rounded-xl shadow border border-gray-200 dark:border-gray-800 mt-8">
      <Toaster position="top-right" />

      {/* Header with back button */}
      <div className="border-b border-gray-200 dark:border-gray-800 px-6 py-4 bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-gray-900 dark:to-gray-800">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate(-1)}
            className="p-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
          >
            <ChevronLeft className="h-5 w-5" />
          </button>
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              {order.title}
            </h2>
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm text-gray-600 dark:text-gray-300">
              <span className="flex items-center gap-1">
                <File className="h-4 w-4" /> #{order.id}
              </span>
              <span className="flex items-center gap-1">
                <User className="h-4 w-4" /> {order.writer}
              </span>
              <span className="flex items-center gap-1">
                <Calendar className="h-4 w-4" /> Submitted:{" "}
                {order.submittedDate}
              </span>
              <span className="flex items-center gap-1">
                <DollarSign className="h-4 w-4" /> {order.price}
              </span>
            </div>
          </div>
        </div>
      </div>

      {/* Status */}
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-500 dark:text-gray-400">Status:</span>
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-300">
              {order.status}
            </span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-500 dark:text-gray-400">
              Revision Due:
            </span>
            <span className="font-medium">{order.revisionDueDate}</span>
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div className="border-b border-gray-200 dark:border-gray-800">
        <nav className="flex -mb-px">
          <button
            onClick={() => setActiveTab("details")}
            className={`px-4 py-3 text-sm font-medium flex items-center gap-2 ${
              activeTab === "details"
                ? "border-b-2 border-blue-500 text-blue-600 dark:text-blue-400"
                : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"
            }`}
          >
            <Info className="h-4 w-4" />
            Revision Details
          </button>
          <button
            onClick={() => setActiveTab("messages")}
            className={`px-4 py-3 text-sm font-medium flex items-center gap-2 ${
              activeTab === "messages"
                ? "border-b-2 border-blue-500 text-blue-600 dark:text-blue-400"
                : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"
            }`}
          >
            <MessageSquare className="h-4 w-4" />
            Messages
          </button>
          <button
            onClick={() => setActiveTab("files")}
            className={`px-4 py-3 text-sm font-medium flex items-center gap-2 ${
              activeTab === "files"
                ? "border-b-2 border-blue-500 text-blue-600 dark:text-blue-400"
                : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"
            }`}
          >
            <Paperclip className="h-4 w-4" />
            Files
          </button>
        </nav>
      </div>

      {/* Tab content */}
      <div className="p-6">
        {activeTab === "details" && (
          <div className="space-y-6">
            <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg border border-amber-100 dark:border-amber-800">
              <div className="flex items-start gap-3">
                <AlertTriangle className="h-5 w-5 text-amber-500 dark:text-amber-400 mt-0.5 flex-shrink-0" />
                <div>
                  <h3 className="font-medium text-gray-900 dark:text-white mb-1">
                    Revision Request
                  </h3>
                  <p className="text-gray-600 dark:text-gray-300">
                    {order.revisionRequest}
                  </p>
                </div>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                  Timeline
                </h3>
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between text-sm text-gray-600 dark:text-gray-300 mb-1">
                      <span>Original Due Date</span>
                      <span>{order.originalDueDate}</span>
                    </div>
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1">
                      <div
                        className="h-full rounded-full bg-green-500"
                        style={{ width: "100%" }}
                      ></div>
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-sm text-gray-600 dark:text-gray-300 mb-1">
                      <span>Revision Submitted</span>
                      <span>{order.submittedDate}</span>
                    </div>
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1">
                      <div
                        className="h-full rounded-full bg-blue-500"
                        style={{ width: "100%" }}
                      ></div>
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-sm text-gray-600 dark:text-gray-300 mb-1">
                      <span>Revision Due Date</span>
                      <span>{order.revisionDueDate}</span>
                    </div>
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1">
                      <div
                        className="h-full rounded-full bg-yellow-500"
                        style={{ width: "100%" }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>

              <div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                  Your Feedback
                </h3>
                {selectedAction ? (
                  <div
                    className={`p-4 rounded-lg ${
                      selectedAction === "approved"
                        ? "bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800"
                        : selectedAction === "rejected"
                        ? "bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800"
                        : "bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800"
                    }`}
                  >
                    <div className="flex items-center gap-2 mb-2">
                      {selectedAction === "approved" ? (
                        <>
                          <ThumbsUp className="h-5 w-5 text-green-500 dark:text-green-400" />
                          <span className="font-medium">Approved</span>
                        </>
                      ) : selectedAction === "rejected" ? (
                        <>
                          <ThumbsDown className="h-5 w-5 text-red-500 dark:text-red-400" />
                          <span className="font-medium">Rejected</span>
                        </>
                      ) : (
                        <>
                          <AlertCircle className="h-5 w-5 text-purple-500 dark:text-purple-400" />
                          <span className="font-medium">Disputed</span>
                        </>
                      )}
                    </div>
                    {revisionNotes && (
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        {revisionNotes}
                      </p>
                    )}
                  </div>
                ) : (
                  <div className="space-y-4">
                    <textarea
                      className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                      rows={4}
                      placeholder="Add your revision notes or feedback..."
                      value={revisionNotes}
                      onChange={(e) => setRevisionNotes(e.target.value)}
                    />
                    <div className="flex gap-3 justify-end">
                      <button
                        onClick={handleApprove}
                        disabled={isSubmitting}
                        className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50"
                      >
                        <Check className="h-4 w-4 mr-2" />
                        {isSubmitting && selectedAction === "approved"
                          ? "Approving..."
                          : "Approve"}
                      </button>
                      <button
                        onClick={handleReject}
                        disabled={isSubmitting}
                        className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50"
                      >
                        <X className="h-4 w-4 mr-2" />
                        {isSubmitting && selectedAction === "rejected"
                          ? "Rejecting..."
                          : "Reject"}
                      </button>
                      <button
                        onClick={handleDispute}
                        disabled={isSubmitting}
                        className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:opacity-50"
                      >
                        <AlertTriangle className="h-4 w-4 mr-2" />
                        {isSubmitting && selectedAction === "disputed"
                          ? "Submitting..."
                          : "Mark as Dispute"}
                      </button>
                    </div>
                  </div>
                )}
              </div>
            </div>
          </div>
        )}

        {activeTab === "messages" && (
          <div className="space-y-6">
            <div className="space-y-4">
              {order.messages.map((message) => (
                <div
                  key={message.id}
                  className={`p-4 rounded-lg ${
                    message.isCustomer
                      ? "bg-blue-50 dark:bg-blue-900/30"
                      : "bg-gray-50 dark:bg-gray-800"
                  }`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-medium text-gray-900 dark:text-white">
                      {message.sender}
                    </span>
                    <span className="text-xs text-gray-500 dark:text-gray-400">
                      {message.timestamp}
                    </span>
                  </div>
                  <p className="text-gray-600 dark:text-gray-300">
                    {message.content}
                  </p>
                </div>
              ))}
            </div>

            <div className="border-t border-gray-200 dark:border-gray-800 pt-4">
              <form className="space-y-4">
                <div>
                  <label htmlFor="message" className="sr-only">
                    Message
                  </label>
                  <textarea
                    id="message"
                    rows={4}
                    className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                    placeholder="Type your message here..."
                    defaultValue={""}
                  />
                </div>
                <div className="flex justify-end gap-3">
                  <button
                    type="button"
                    className="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 shadow-sm text-sm font-medium rounded-md text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                  >
                    <Edit className="h-4 w-4 mr-2" />
                    Request Edit
                  </button>
                  <button
                    type="submit"
                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                  >
                    Send Message
                  </button>
                </div>
              </form>
            </div>
          </div>
        )}

        {activeTab === "files" && (
          <div className="space-y-6">
            <div className="space-y-4">
              {order.attachments.map((file, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg"
                >
                  <div className="flex items-center gap-4">
                    <div className="p-3 rounded-full bg-gray-100 dark:bg-gray-800">
                      <File className="h-5 w-5 text-gray-500 dark:text-gray-400" />
                    </div>
                    <div>
                      <h4 className="font-medium text-gray-900 dark:text-white">
                        {file.name}
                      </h4>
                      <div className="flex gap-4 text-sm text-gray-500 dark:text-gray-400 mt-1">
                        <span>{file.size}</span>
                        <span>Uploaded: {file.date}</span>
                      </div>
                    </div>
                  </div>
                  <button
                    onClick={() => handleDownload(file)}
                    className="p-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
                    title="Download file"
                  >
                    <Download className="h-5 w-5" />
                  </button>
                </div>
              ))}
            </div>

            <div className="border-t border-gray-200 dark:border-gray-800 pt-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                  Upload Additional Files
                </h3>
                <button className="inline-flex items-center px-3 py-1.5 border border-gray-300 dark:border-gray-600 shadow-sm text-sm font-medium rounded-md text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                  Upload
                </button>
              </div>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Upload any additional materials or reference documents.
              </p>
            </div>
          </div>
        )}
      </div>

      {/* Action buttons */}
      <div className="border-t border-gray-200 dark:border-gray-800 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
        <div className="flex flex-wrap gap-3 justify-between">
          <div className="flex gap-3">
            <button
              className="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 shadow-sm text-sm font-medium rounded-md text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              onClick={handleChat}
            >
              <MessageSquare className="h-4 w-4 mr-2" />
              Message Writer
            </button>
            <button
              className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              onClick={() => handleDownload(order.attachments[0])}
            >
              <Download className="h-4 w-4 mr-2" />
              Download All Files
            </button>
          </div>
          <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            <Clock className="h-4 w-4" />
            <span>Last updated: {order.lastUpdated}</span>
          </div>
        </div>
      </div>
    </div>
  );
}
